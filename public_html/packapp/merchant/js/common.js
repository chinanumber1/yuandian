...
/*全局检测登录态*/
if(visitPage != 'login' && visitPage != 'index' && visitPage != 'reg' && visitPage != 'forgot_password'){
	common.checkLogin();
}
...
function iosFixedTop(){
	if(common.checkIosApp()){
		...
		$('.mui-bar-nav').addClass('iosHeader');
		var editClass = [];
		// $('body').prepend('<div class="ios_fixed_top"></div>');

		// editClass.push({class:'.mui-bar-nav',css:'top'});
		if(visitPage == 'fastList'){
			editClass.push({class:'.store',css:'padding-top'});	
			editClass.push({class:'.h130 ',css:'padding-top'});	
		}else if(visitPage == 'foodList'){
			editClass.push({class:'.reserve .reserve_tab ',css:'top'});	
			editClass.push({class:'.reserve ',css:'padding-top'});	
		}else if(visitPage == 'group_buy'){
			editClass.push({class:'.cable',css:'top'});	
			//editClass.push({class:'..allGoods',css:'padding-top'});	
		}else if(visitPage == 'grouping_buy_lists'){
			editClass.push({class:'.cable',css:'top'});	
			editClass.push({class:'.order .order_list',css:'margin-top'})
		}else if(visitPage == 'group_detail'){
			editClass.push({class:'.p40',css:'margin-top'})
		}else if(visitPage == 'booking_order'){
			editClass.push({class:'.cable',css:'top'});	
			editClass.push({class:'.fix',css:'top'});
			editClass.push({class:'.mui-scroll-wrapper',css:'top'});	
		}else if(visitPage == 'order_detail'){
			editClass.push({class:'.h44',css:'height'});
		}else if(visitPage == 'fastList_detail'){
			editClass.push({class:'.p40',css:'margin-top'});
		}else if(visitPage == 'plus' ){
			editClass.push({class:'.fixed_header',css:'top'});
			editClass.push({class:'.select_input i',css:'top'});
		}
...
$('.left_style').click(function(){
	if($(this).data('tip_content')){
		mui.alert($(this).data('tip_content'),$(this).data('tip_title') ? $(this).data('tip_title') : '说明','知道了');
		if($('.mui-popup').size() == 1){
			$('.mui-popup').addClass('popup-text-left');
		}
	}else if($(this).siblings('input').length == 1){
...
// 点击返回按钮
$('.mui-icon-left-nav').click(function(e){
	e.stopPropagation();
	e.preventDefault(); 
