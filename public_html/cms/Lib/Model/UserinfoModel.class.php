...
// 		} else return false;
		return D('Card_new')->user_score($uid, $mer_id, $score, $desc );
	}
	
	/*获取积分*/
	public function get_score($uid, $mer_id)
	{
		$user = D('Card_new')->get_card_by_uid_and_mer_id($uid,$mer_id);
		return isset($user['card_score']) ? intval($user['card_score']) : 0;
	}
}
?>