...
class Merchant_categoryModel extends Model{
    ...
    public function lists()
    {
        ...
        $items = $this->field(true)->where($where)->order('`cat_sort` DESC')->select();
        $tmpMap = array();
        ...
        foreach ($items as $item) {
            if (isset($tmpMap[$item['cat_fid']])) {
                $tmpMap[$item['cat_fid']]['children'][$item['cat_id']] = &$tmpMap[$item['cat_id']];
            } else {
                $list[$item['cat_id']] = &$tmpMap[$item['cat_id']];
            }
        }
        $result = array();
        foreach ($list as $l) {
            ...
            $value = array();
            $value['value'] = $l['cat_id'];
            $value['text'] = $l['cat_name'];
            $value['children'] = $this->formatList($l['children']);
            $result[] = $value;
        }
