...
class Group_categoryModel extends Model{
	...
	public function get_hotel_category(){
		$where['is_hotel'] = '1';
		$where['cat_status'] = 1;
		$hotel_category = $this->field('cat_id')->where($where)->select();
		foreach ($hotel_category as $item) {
			$hotel_cate_fid[] = $item['cat_id'];
		}

//		$condition_hotel_category['cat_status'] = '1';
//		$condition_group_category['cat_id'] = array('in',$hotel_cate_fid);
//		$condition_group_category['cat_fid'] = array('in',$hotel_cate_fid);
//		$condition_group_category['_logic'] = "or";
//		$condition_hotel_category['_complex'] = $condition_group_category;
//		$condition_hotel_category['_logix'] = "and";

		$condition_hotel_category = "cat_status = 1 AND ( cat_id in (".implode(',',$hotel_cate_fid).") OR cat_fid in (".implode(',',$hotel_cate_fid).") ) ";
		$tmp_group_category = $this->field(true)->where($condition_hotel_category)->order('`cat_sort` DESC')->limit()->select();
