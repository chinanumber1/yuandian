...
class ClassifyAction extends BaseAction {
    ...
    public $collectUrl = 'http://o2oclassfy-server.bupu.net';
	
	protected function clearCache(){
		S('classify_hot_category',null);
	}
            ...
            if($_FILES){
                if($_FILES['cat_pic']['error'] != 4){
                    ...
                    if($upload->upload()){
                        ...
                        $datas['cat_pic'] = $rand_num.'/'.$uploadList[0]['savename'];
                    }else{
    ...
    public function cat_field_modify() {
        /*         * type值：1单文本框2单选框3复选框4下拉框5多文本框6地图(地图只能添加一次)****** */
        if (IS_POST) {
            $Classify_categoryDB = D('Classify_category');
            $condition_now_Classify_category['cid'] = intval($_POST['cid']);
            ...
            if (!empty($now_category['cat_field'])) {
                ...
                foreach ($cat_field as $key => $value) {
                    if($value['type'] == $_POST['type'] && $_POST['type']== 6){
                        $this->error('该类型只能添加一次,请勿重复添加!');
                    }
            ...
            if (empty($_POST['use_field'])) {
                ...
                // if ($post_data['type'] == 6) {
                //     $post_data['inarr'] = intval($_POST['inarr']) ?　intval($_POST['inarr']) : 0;
                //     $post_data['inunit'] = trim($_POST['inunit']) ? trim($_POST['inunit']) : '';
                // }
    ...
    public function checkList() {
        ...
        if($_GET['province_idss'] && $this->config['many_city']){
            ...
            $wherestr['city_id']=array('in',$city_arr);
        }

        if($_GET['city_idss']){
            $area_id= $_GET['city_idss'];
            $wherestr['city_id']=$area_id;
        }
    ...
    public function infoList() {
        ...
        if($_GET['province_idss'] && $this->config['many_city']){
            ...
            $wherestr.=' AND `city_id` in ('.implode(',',$city_arr).') ';
        }

        if($_GET['city_idss']){
            $area_id= $_GET['city_idss'];
            $wherestr.=' AND `city_id`='.$area_id;
        }
    ...
    public function toVerify() {
        $vid = intval($_POST['vid']);
        if ($vid > 0) {        
            $info = M('Classify_userinput')->where(array('id' => $vid))->find();
			if(empty($info)){
				$this->error('此信息不存在');
			}
			$res = M('Classify_userinput')->where(array('id' => $vid))->save(array('status' => 1));
            if ($res) {
                $now_user = M('User')->field(true)->where(array('uid'=>$info['uid']))->find();
				
				if(!M('Classify_scrollmsg')->where(array('classify_id'=>$vid))->find()){
					$classify_category_info = M('Classify_category')->where(array('cid'=>$info['cid']))->find();
					$data_scrollmsg = array(
						'classify_id'=> $vid,
						'time' 		 => time(),
						'text' 		 => $now_user['nickname'].'发布了'.$classify_category_info['cat_name'].'信息',
					);
                ...
                if ($now_user['openid']) {
                    ...
                    $model->sendTempMsg('TM00017', array(
                        ...
                        'first' => '您发布的标题为【'.$info['title'].'】的信息已审核',
                        'OrderSn' => $vid,
                        'OrderStatus' => '审核通过',
    ...
    public function replyList() {
        ...
        $wherestrQuery = '`r`.`vid`='.$vid.' AND `r`.`uid`=`u`.`uid` AND `r`.`status`=0';
    ...
    public function reply_detail() {
        ...
        $wherestrQuery = '`r`.`reply_id`='.$reply_id.' AND `r`.`uid`=`u`.`uid` AND `r`.`status`=0';


        $detail = M('')->field('`r`.*,`u`.`nickname`,`u`.`avatar`')->table(array(C('DB_PREFIX').'classify_reply'=>'r',C('DB_PREFIX').'user'=>'u'))->where($wherestrQuery)->find();
    ...
    public function reply_del() {
        if (IS_POST) {
            $database_Classify_reply = D('Classify_reply');
            $where['reply_id'] = intval($_POST['reply_id']);
            $now_reply = $database_Classify_reply->field(true)->where($where)->find();
            if ($now_reply && $now_reply['status'] == 0) {
                if ($database_Classify_reply->where($where)->data(array('status' => 1))->save()) {
                    $this->success('删除成功！');
                } else {
                    $this->error('删除失败！请重试~');
                }
            } else {
                $this->error('该数据不存在或者已删除~');
            }
        } else {
            $this->error('非法提交,请重新提交~');
        }
    ...
    public function assure_order_list(){
        ...
        $condition_table = array(C('DB_PREFIX').'classify_order'=>'o', C('DB_PREFIX').'user'=>'u',C('DB_PREFIX').'classify_userinput'=>'c');
        $condition_where = '`o`.`uid` = `u`.`uid` AND `o`.`classify_userinput_id`  = `c`.`id` ';
        ...
        if($_GET['province_idss'] && $this->config['many_city']){
            ...
            $condition_where.=' AND `c`.`city_id` in ('.implode(',',$city_arr).') ';
        }

        if($_GET['city_idss']){
            $area_id= $_GET['city_idss'];
            $condition_where.=' AND `c`.`city_id` = '.$area_id;
        }
	...
	public function config(){
        redirect(U('Config/index',array('galias'=>'classify','header'=>'Classify/config')));
    }
}
