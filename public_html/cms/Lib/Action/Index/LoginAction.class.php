...
class LoginAction extends BaseAction {
    ...
    public function reg() {
        if (IS_POST) {
            ...
            $result = D('User')->checkreg($phone, $pwd,$_POST['phone_country_type']);
    ...
    public function Generate() {
        ...
        if ($Userarr && ($_POST['change_phone']==1 || $_POST['reg']==1))
            exit(json_encode(array('error_code' => 3, 'msg' => '手机账号已注册！')));
        if($this->config['sms_verify_fleshcode']){
            if (empty($flesh) || empty($_POST['type']) ){
                exit(json_encode(array('error_code' => 1, 'msg' => '请先验证验证码')));
            }else{
