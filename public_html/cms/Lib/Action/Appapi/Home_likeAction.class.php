...
class Home_likeAction extends BaseAction{
	...
	public function index(){
		...
		if($content_type=='group'){
			...
			$group_count = M('Group')->join('AS g INNER JOIN '.C('DB_PREFIX').'merchant m ON g.mer_id = m.mer_id ')->where("`m`.`city_id`='". C('config.now_city')."'  AND `g`.`status`='1' AND `m`.`status`='1' AND `g`.`type`='1' AND `g`.`begin_time`<'$now_time' AND `g`.`end_time`>'$now_time'")->count();

			$new_group_list = D('Group')->get_group_list('index_sort',($page-1)*$page_count.','.$page_count,true);

			$page_all = ceil($group_count/$page_count);
			//判断是否微信浏览器，
			$group_list = array();
				foreach($new_group_list as $storeGroupValue){
					...
					$tmp['price'] = strval($storeGroupValue['price']);
					$tmp['old_price'] = strval($storeGroupValue['old_price']);
			...
			$cat_fid = 0;
			if($_POST['city_id']!=''){
				C('config.now_city',$_POST['city_id']);
			}
			...
			$key && $where['key'] = $key;
			//一次查询返回10个
			$lists = D('Merchant_store_shop')->get_list_by_option($where,5);
			$page_all = $lists['pagetotal'] ? $lists['pagetotal'] : 0;
			$page_all = $page_all>$page_max?$page_max:$page_all;
			$return = array();
			$now_time = date('H:i:s');
			$n= 1;
			...
			foreach ($lists['shop_list'] as $row) {
				...
				$temp['time'] = D('Merchant_store_shop')->getBuniessName($row);
				//背景色
				$temp['shop_fitment_color'] = $row['shop_fitment_color'] ? $row['shop_fitment_color'] : '113533';
				//标题色
				$temp['shop_fitment_front_color'] = 'ffffff';
...
//			}
		...
		} elseif ($content_type == 'mall') {
		    $flist = D('Goods_category')->field('id')->where(array('fid' => 0, 'is_hot' => 1))->select();
		    $fids = array();
		    ...
		    $data = array();
		    if ($fids) {
		        $list = D('Goods_category')->field(true)->where(array('fid' => array('in', $fids), 'is_hot' => 1))->select();
		        foreach ($list as $val) {
		            if (isset($data[$val['fid']])) {
		                $data[$val['fid']][] = $val;
		            } else {
		                $return = $this->getData($val['id']);
		                $val['goodslist'] = isset($return['data']) ? $return['data'] : array();
            ...
            $page_all = $return['totalPage'] ? $return['totalPage'] : 0;
            $page_all = $page_all > $page_max ? $page_max : $page_all;
            $n = 1;
            $newList = array();
            foreach ($return['store_list'] as $storeValue) {
                ...
                if (empty($storeValue['pingjun'])) {
                    $storeValue['pingjun'] = 0;
                }
//                 $storeValue['discount_txt'] = $storeValue['discount_txt'] ? unserialize($storeValue['discount_txt']) : array();
                
                if ($storeValue['discount_txt']) {
                    $discount_txt = unserialize($storeValue['discount_txt']);
                    if ($discount_txt['discount_type'] == 1) {
                        $storeValue['discount_txt'] = $discount_txt['discount_percent'] . '折';
                    } else {
                        $storeValue['discount_txt'] = '每满' . $discount_txt['condition_price'] . '减' . $discount_txt['minus_price'] . '元';
                    }
                } else {
                    $storeValue['discount_txt'] = '';
                }
                
                $storeValue['url'] = $this->config['site_url'] . str_replace('appapi.php', 'wap.php', U('Merchant/shop', array('store_id' => $storeValue['store_id'])));
...
//						$returnObj[$key][] = '商品满'.$obj[$key][$k]['money'].'元,配送费减'.$obj[$key][$k]['minus'].'元';
						$returnObj[$key][] = '满'.$obj[$key][$k]['money'].'配送减'.$obj[$key][$k]['minus'];
					} else {
//						$returnObj[$key][] = '满'.$obj[$key][$k]['money'].'元减'.$obj[$key][$k]['minus'].'元';
						$returnObj[$key][] = $obj[$key][$k]['money'].'减'.$obj[$key][$k]['minus'];
					}
				}
			}
		}
...
//		    if($key=='invoice' || $key=='discount' || $key=='isdiscountgoods' || $key=='isdiscountsort'){
		    if($key=='invoice' || $key=='discount' || $key=='isdiscountgoods'){
				$textObj[$key] = $value;
			}else{
				switch($key){
					case 'system_newuser':
    ...
    public function mall()
    {
        ...
        $this->returnCode(0, array('data' => $data));
    }
...
//         exit(json_encode(array('error' => 0, 'data' => $return, 'fid' => $fid)));
    ...
    private function getData($cateId)
    {
        ...
        foreach ($goodsList as $goods) {
            ...
            $goods['image'] = $image;
            $goods['score_mean'] = round($goods['score_mean']/5 * 100, 2) . '%';
            $goods['url'] = $this->config['site_url'] . '/wap.php?c=Mall&a=detail&goods_id=' . $goods['goods_id'];
            $return[] = $goods;
        }
        return array('data' => $return, 'fid' => $fid);
    }
