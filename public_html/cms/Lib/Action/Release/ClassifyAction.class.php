...
class ClassifyAction extends BaseAction {
    ...
    public function Lists($data = false) {
        ...
        if ($cid > 0) {
            ...
            $userinputDb = M('Classify_userinput');
            if (isset($_GET['keystr']) && !empty($_GET['keystr'])) {
                $wherestr.='status=1 AND (title LIKE "%' . $_GET['keystr'] . '%" OR input1 LIKE "%' . $_GET['keystr'] . '%" OR input2 LIKE "%' . $_GET['keystr'] . '%" OR input3 LIKE "%' . $_GET['keystr'] . '%" OR input4 LIKE "%' . $_GET['keystr'] . '%")';
                $this->assign('qstr', $_GET['keystr']);
            }
            ...
            $this->assign('listsdatas', $tmpdatas);
            $this->display('searchClassifyLists');
            exit();
        }
    }
    ...
    public function fabuTosave() {
        ...
        if (!$this->config['international_phone'] && !preg_match('/^[0-9]{5,20}$/', $lxtel))
        ...
        $userinputDb = M('Classify_userinput');
        if($_POST['id']){
            $userinputDb->where(array('id'=>$_POST['id']))->save($insert_datas);
            $insert_id =$_POST['id'];
        }else{
        ...
        if ($insert_id > 0) {
            ...
            if($this->config['fabu_classify_score_add']>0){
                ...
                D('Scroll_msg')->add_msg('classify',$this->user_session['uid'],'用户'.str_replace_name($this->user_session['nickname']).'于'.date('Y-m-d H:i',$_SERVER['REQUEST_TIME']).'发布分类信息获得'.$this->config['fabu_classify_score_add'].'个'.$this->config['score_name']);
