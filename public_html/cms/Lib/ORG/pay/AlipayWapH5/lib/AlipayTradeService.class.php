...
class AlipayTradeService
{
    ...
    {
        $response = $this->aopclientRequestExecute($data, true);
        return $response;
    }

    private function aopclientRequestExecute($data, $ispage = false)
    {
        $aop = new AopClient();
        ...
        $aop->debugInfo = true;
        if ($ispage) {
            $result = $aop->pageExecute($data, "post");
            echo $result;
        } else {
            $result = $aop->execute($data);
        }
        return $result;
    }
    ...
    {
        $response = $this->aopclientRequestExecute($data);
     ...
     * @return $response 支付宝返回的信息
     */
    public function downloadurlQuery($data)
    {
        $response = $this->aopclientRequestExecute($data);
