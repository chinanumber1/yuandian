...
class AopClient
{
    ...
    protected function getSignContent($params)
    {
        ...
        foreach ($params as $k => $v) {
            if (false === $this->checkEmpty($v) && "@" != substr($v, 0, 1)) {
                ...
                if ($i == 0) {
                    $stringToBeSigned .= "$k" . "=" . htmlspecialchars_decode($v);
                } else {
                    $stringToBeSigned .= "&" . "$k" . "=" . htmlspecialchars_decode($v);
                }
                $i ++;
    ...
    public function sdkExecute($data)
    {
        ...
        if ($notify_url = $data['notify_url']) {
            $params['notify_url'] = $notify_url;
        }
        
        $dict = ['biz_content' => $data['biz_content']];
     ...
     * auther:笙默
     */
    public function pageExecute($data, $httpmethod = "POST")
    {
        $this->setupCharsets($data);
        ...
        if (!$this->checkEmpty($data['api_version'])) {
            $iv = $data['api_version'];
        } else {
            $iv = $this->apiVersion;
        }
        ...
        $sysParams["method"] = $data['method'];
        $sysParams["timestamp"] = date("Y-m-d H:i:s");
        $sysParams["alipay_sdk"] = $this->alipaySdkVersion;
        $sysParams["terminal_type"] = isset($data['terminal_type']) ? $data['terminal_type'] : '';
        $sysParams["terminal_info"] = isset($data['terminal_info']) ? $data['terminal_info'] : '';
        $sysParams["prod_code"] = isset($data['prod_code']) ? $data['prod_code'] : '';
        $sysParams["notify_url"] = $data['notify_url'];
        $sysParams["return_url"] = $data['return_url'];
        $sysParams["charset"] = $this->postCharset;
        
        // 获取业务参数
        $apiParams = ['biz_content' => $data['biz_content']];
        
        if (isset($data['need_encrypt']) && $data['need_encrypt']) {
    ...
    public function execute($data, $authToken = null, $appInfoAuthtoken = null)
    {
        $this->setupCharsets($data);
        ...
        $sysParams["terminal_info"] = isset($data['terminal_info']) ? $data['terminal_info'] : '';
        $sysParams["prod_code"] = isset($data['prod_code']) ? $data['prod_code'] : '';
        $sysParams["notify_url"] = isset($data['notify_url']) ? $data['notify_url'] : '';
...
//             $this->logCommunicationError($sysParams["method"], $requestUrl, "HTTP_ERROR_" . $e->getCode(), $e->getMessage());
        ...
        if ("json" == $this->format) {
            ...
            if (null !== $respObject) {
                $respWellFormed = true;
                $signData = $this->parserJSONSignData($data, $resp, $respObject);
            }
            ...
            if (false !== $respObject) {
                ...
                $signData = $this->parserXMLSignData($data, $resp);
            }
        }
        ...
        // 验签
        $this->checkResponseSign($data, $signData, $resp, $respObject);
        
        // 解密
        if (isset($data['need_encrypt']) && $data['need_encrypt']) {    
            if ("json" == $this->format) {
                
                $resp = $this->encryptJSONSignSource($data, $resp);
                ...
                $resp = $this->encryptXMLSignSource($data, $resp);
    ...
    function parserResponseSubCode($data, $responseContent, $respObject, $format)
    {
        if ("json" == $format) {
    ...
    function parserJSONSignData($data, $responseContent, $responseJSON)
    {
        $signData['sign'] = $this->parserJSONSign($responseJSON);
        $signData['signSourceData'] = $this->parserJSONSignSource($data, $responseContent);
        
        return $signData;
    }

    function parserJSONSignSource($data, $responseContent)
    {
        $apiName = $data['method'];
        $rootNodeName = str_replace(".", "_", $apiName) . $this->RESPONSE_SUFFIX;
    ...
    function parserXMLSignData($data, $responseContent)
    {
        
        $signData['sign'] = $this->parserXMLSign($responseContent);
        $signData['signSourceData'] = $this->parserXMLSignSource($data, $responseContent);
        
        return $signData;
    }

    function parserXMLSignSource($data, $responseContent)
    {
        $apiName = $data['method'];
        $rootNodeName = str_replace(".", "_", $apiName) . $this->RESPONSE_SUFFIX;
     ...
     * @throws Exception
     */
    public function checkResponseSign($data, $signData, $resp, $respObject)
    {
        if (! $this->checkEmpty($this->alipayPublicKey) || ! $this->checkEmpty($this->alipayrsaPublicKey)) {
            ...
            // 获取结果sub_code
            $responseSubCode = $this->parserResponseSubCode($data, $resp, $respObject, $this->format);
            ...
            if (! $this->checkEmpty($responseSubCode) || ($this->checkEmpty($responseSubCode) && ! $this->checkEmpty($signData['sign']))) {
                ...
                if (! $checkResult) {
                    ...
                    if (strpos($signData['signSourceData'], "\\/") > 0) {
                        ...
                        if (! $checkResult) {
                            throw new Exception("1.check sign Fail! [sign=" . $signData['sign'] . ", signSourceData=" . $signData['signSourceData'] . "]");
                        }
                    } else {
                        
                        throw new Exception("2.check sign Fail! [sign=" . $signData['sign'] . ", signSourceData=" . $signData['signSourceData'] . "]");
                    }
                }
            }
        }
    ...
    private function setupCharsets($data)
    {
        if ($this->checkEmpty($this->postCharset)) {
            $this->postCharset = 'UTF-8';
        }
        $str = preg_match('/[\x80-\xff]/', $this->appId) ? $this->appId : print_r($data, true);
        $this->fileCharset = mb_detect_encoding($str, "UTF-8, GBK") == 'UTF-8' ? 'UTF-8' : 'GBK';
    }
    
    // 获取加密内容
    private function encryptJSONSignSource($data, $responseContent)
    {
        $parsetItem = $this->parserEncryptJSONSignSource($data, $responseContent);
    ...
    private function parserEncryptJSONSignSource($data, $responseContent)
    {
        $apiName = $data['method'];
        $rootNodeName = str_replace(".", "_", $apiName) . $this->RESPONSE_SUFFIX;
    ...
    // 获取加密内容
    private function encryptXMLSignSource($data, $responseContent)
    {
        $parsetItem = $this->parserEncryptXMLSignSource($data, $responseContent);
    ...
    private function parserEncryptXMLSignSource($data, $responseContent)
    {
        $apiName = $data['method'];
        $rootNodeName = str_replace(".", "_", $apiName) . $this->RESPONSE_SUFFIX;
