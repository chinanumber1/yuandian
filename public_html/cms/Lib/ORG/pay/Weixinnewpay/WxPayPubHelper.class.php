...
class JsApi_pub extends Common_util_pub
{
	 ...
	 * 红包申请接口
	 */
	class RedPack extends Wxpay_client_pub
	{
		function __construct($appid,$mchid,$key,$appsecret)
		{
			//设置接口链接
		...
		function createXml()
		{
			...
			{
				//检测必填参数
				if($this->parameters["total_amount"] == null){
					throw new SDKRuntimeException("红包申请接口中，缺少必填参数total_amount！"."<br>");
				}
				$this->parameters["wxappid"] = $this->appid;//公众账号ID
				$this->parameters["mch_id"] = $this->mchid;//商户号
				$this->parameters["nonce_str"] = $this->createNoncestr();//随机字符串
				$this->parameters["client_ip"] = get_client_ip();//终端ip
				$this->parameters["act_name"] = '抽奖获得红包';//终端ip
				$this->parameters["sign"] = $this->getSign($this->parameters);//签名
				return  $this->arrayToXml($this->parameters);
			}catch (SDKRuntimeException $e)
			{
