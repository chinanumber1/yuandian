		...
		<div class="mainbox">
				...
				<div id="nav" class="mainnav_title">
					<ul>
						...
						<a href="{pigcms{:U('setAccountDeposit/withdraw_apply')}" class="on">平台云商通提现</a>
					</ul>
				</div>
			...
			<form id="myform" method="post" action="{pigcms{:U('submitVerify')}" refresh="true">
				...
				<div class="headings gengduoxian">您的余额为：￥{pigcms{$now_money} </div>
				<tr>
					<th width="160">金额</th>
						<td>
						<input type="text" class="input-text" name="money" id="money" value="" />元&nbsp;<label id="percent">(最低提现额：{pigcms{$config['min_withdraw_money']}元)</label>
						</td>
					</tr>
					
					<th width="160">备注(限定50字符内)</th>
						<td>
						<textarea name="extendInfo" id="extendInfo" class="input-text" style="width:410px;height:120px;"></textarea>
						</td>
					</tr>
			   ...
			   <div class="btn" style="margin-top:20px;"> 
				<input type="submit" name="dosubmit" value="提现" class="button" /> 
					...
					if($("input[name='money']").val()==''){
						...
						if(countdown==60){
							$.ajax({
								...
								type: 'POST',
								dataType: 'json',
								data: {money: $("input[name='money']").val(),extendInfo:$('#extendInfo').val()},
