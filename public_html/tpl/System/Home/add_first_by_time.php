<include file="Public:header"/>
		<div class="mainbox">
			<div id="nav" class="mainnav_title">
				<ul>
					<a href="{pigcms{:U('Home/first_by_time')}" >分时段关注自动回复</a>
			...
			<form method="post" action="" refresh="true" enctype="multipart/form-data" >
				<table cellpadding="0" cellspacing="0" class="table_form" width="100%">
					...
					<if condition="$_GET['id'] gt 0"><input type="hidden" name="id" value="{pigcms{$_GET['id']}"></if>
					<tr >
						<th width="160">日期筛选</th>
						<td><input type="text" class="input-text" name="start_time" style="width:120px;" id="d4311"  value="{pigcms{$first.start_time}" onfocus="WdatePicker({readOnly:true,dateFmt:'HH:mm:00'})"/>			   
							<input type="text" class="input-text" name="end_time" style="width:120px;" id="d4311" value="{pigcms{$first.end_time}" onfocus="WdatePicker({readOnly:true,dateFmt:'HH:mm:00'})"/></td>
					</tr>
				
							
					<tr class="class_0" <if condition="$first['type'] neq 0">style="display:none"</if>>
						<th width="160">回复内容　</th>
						<td><textarea rows="20" cols="45" name="content" id="content">{pigcms{$first.content}</textarea><img src="./static/images/exp1.png"></td>
					</tr>
