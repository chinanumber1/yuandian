        ...
        if ($(this).hasClass('lock')) {
            ...
            if (userOrders[id] != undefined && userOrders[id].length > 0) {
                $.each(userOrders[id], function(i, item){
                    map.removeOverlay(userSendLabel[item.supply_id]);
                    map.removeOverlay(userFetchLabel[item.supply_id]);
                    map.removeOverlay(userPolineArray[item.supply_id]);
                });
            }
        } else {
            ...
            map.panTo(new BMap.Point(sendUsers[id].now_lng, sendUsers[id].now_lat), 15);//TODO
            ...
            if (userOrders[id] != undefined && userOrders[id].length > 0) {
                $.each(userOrders[id], function(i, item){
                    userSendLabel[item.supply_id] = new BMap.Label('<div class="amap-marker-content" style="opacity: 1;"><i class="assigned-icon user"></i></div>', {position:new BMap.Point(item.aim_lnt, item.aim_lat), offset:new BMap.Size(-15, -25)});  // 创建文本标注对象
                    map.addOverlay(userSendLabel[item.supply_id]);
                    
                    userFetchLabel[item.supply_id] = new BMap.Label('<div class="amap-marker-content" style="opacity: 1;"><i class="assigned-icon wait-take-rest"></i></div>', {position:new BMap.Point(item.from_lnt, item.from_lat), offset:new BMap.Size(-15, -25)});  // 创建文本标注对象
                    map.addOverlay(userFetchLabel[item.supply_id]);
                    
                    userPolineArray[item.supply_id] = new BMap.Polyline([
                        new BMap.Point(item.from_lnt, item.from_lat),
                        new BMap.Point(item.aim_lnt, item.aim_lat)
                    ], {strokeColor:"#697694", strokeWeight:2, strokeOpacity:0.5, strokeStyle: 'dashed'});   //创建折线
                    map.addOverlay(userPolineArray[item.supply_id]); 
                });
            }
