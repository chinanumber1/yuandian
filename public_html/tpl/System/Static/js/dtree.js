...
//==================================================
//节点对象  -- Node object
             //437, 423, 'areaIds[]', 437, '潮州', false,'',     '',      '',  '',   '',       ' ',false
function Node(id, pid, cname, cvalue, cshow, cchecked, cdisabled, url, title, target, icon, iconOpen, open) {
	// console.log(id+'--'+cname+'-'+cchecked+'--'+cshow);
	// console.log(id+'-dis--'+cdisabled);
    ...
    this._ioo = open || false;;                             // Node 父节点对象，默认值null。 
};
...
function dTree(objName, objbool) {        // void 构造方法，创建树对象。objName: 树名称。objbool：单选多选 true多选  false单选  默认单选         
    ...
    this.icon = {
        root:        static_path+'css/img/base.gif',         // Node 根节点。 
        folder:      static_path+'css/img/folder.gif',       // Node 根节点。 
        folderOpen:  static_path+'css/img/folderopen.gif',   // 打开的文件夹,默认'img/folderOpen.gif' 
        node:        static_path+'css/img/page.gif',         // 打开的文件夹,默认'img/folderOpen.gif' 
        empty:       static_path+'css/img/empty.gif',        // 打开的文件夹,默认'img/folderOpen.gif' 
        line:        static_path+'css/img/line.gif',         // 竖线,默认'img/line.gif'
        join:        static_path+'css/img/join.gif',         // 丁线,默认'img/join.gif' 
        joinBottom:  static_path+'css/img/joinbottom.gif',   // 直角线,默认'img/joinbottom.gif' 
        plus:        static_path+'css/img/plus.gif',         // 加号丁线,默认'img/plus.gif' 
        plusBottom:  static_path+'css/img/plusbottom.gif',   // 加号直角线,默认'img/plusbottom.gif' 
        minus:       static_path+'css/img/minus.gif',        // 减号丁线,默认'img/minus.gif' 
        minusBottom: static_path+'css/img/minusbottom.gif',  // 减号直角线,默认'img/minusbottom.gif'  
        nlPlus:      static_path+'css/img/nolines_plus.gif', // 减号直角线,默认'img/minusbottom.gif' 
        nlMinus:     static_path+'css/img/nolines_minus.gif' //无线减号,默认'img/nolines_minus.gif' 
    };
...
dTree.prototype.selectAll = function () {
    this.sAll(true);
};
dTree.prototype.unselectAll = function () {
    this.sAll(false);
};
...
dTree.prototype.node = function (node, nodeId) {
    ...
    if (node.pid == this.root.id) {
        ...
        if (node.pid==0) {
            ...
            checkboxSyntax ="<input type='checkbox'	onClick='select_son_all(this);' name='top_node'><span class='dtree_node' node_id='" + node.id + "'>" + node.cshow + "</span>";
        }else {            
            /**组装checkbox开始*/            
            ...
            if (node.cchecked){
                checkboxSyntax += " checked="+'"checked"'+" ";
			}
    ...
    str += '</div>';
    if (node._hc) {
        // str += '<div id="d' + this.obj + nodeId + '" class="clip" style="display:' + ((node.cchecked || node.pid==-1) ? 'block' : 'none') + ';">';
...
dTree.prototype.o = function (id) {
    ...
    var cn = that.aNodes[id];

    cn._ioo = !cn._ioo;
	
    this.nodeStatus(cn._ioo, id, cn._ls);
...
dTree.prototype.oAll = function (status) {
    for (var n = 0; n < this.aNodes.length; n++) {
        if (this.aNodes[n]._hc && this.aNodes[n].pid != this.root.id) {
            this.nodeStatus( this.aNodes[n].cchecked, n, this.aNodes[n]._ls)
...
dTree.prototype.o_All = function (status) {
    for (var n = 0; n < this.aNodes.length; n++) {
        if (this.aNodes[n]._hc && this.aNodes[n].pid != this.root.id) {
...
dTree.prototype.s_All = function (status) {
    for (var n = 0; n < this.aNodes.length; n++) {
        if (this.aNodes[n]._hc && this.aNodes[n].pid != this.root.id) {
			this.checkPNode( this.aNodes[n].id,true)
		}
    }
    if (this.config.useCookies) this.updateCookie();
};
...
dTree.prototype.nodeStatusselect = function (status, id, bottom) {
    eDiv = document.getElementById('d' + this.obj + id);
    eJoin = document.getElementById('j' + this.obj + id);
    if (this.config.useIcons) {
        eIcon = document.getElementById('i' + this.obj + id);
