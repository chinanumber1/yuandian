<include file="Public:header"/>
	<form id="myform" method="post" action="{pigcms{:U('House/open_door_adver_edit')}" enctype="multipart/form-data">
		...
		<table cellpadding="0" cellspacing="0" class="frame_form" width="100%">
			...
			<tr>
				<th width="80">小区名称</th>
				<td><input type="text" class="input fl" name="village_name" id="village_name" size="20" value="{pigcms{$village_info.village_name}" placeholder="请输入名称" validate="maxlength:20,required:true"/></td>
				<input type="hidden" name="village_id" id="village_id" value="{pigcms{$now_adver.village_id}">
			</tr>
			
			<tr <if condition="empty($village_info)">class="hide"</if>>
				<th width="80">小区信息</th>
				<td id="village_info">
					小区id:{pigcms{$village_info.village_id}<br>
					小区名称:{pigcms{$village_info.village_name}<br>
					小区物业:{pigcms{$village_info.property_name}<br>
					小区地址:{pigcms{$village_info.village_address}<br>
				</td>
 			</tr>
			...
			<tr>
				<th width="80">图片展示</th>
				<td><img src="{pigcms{$now_adver.android_pic}" id="img" style="width:200px;height:100px;"></td>
			</tr>
