...
<style>
	...
	<form id="myform" method="post" action="{pigcms{:U('Deliver/user_add')}" frame="true" refresh="true">
		...
		<table cellpadding="0" cellspacing="0" class="frame_form" width="100%">
			...
			<tr>
				<th width="15%">密码</th>
				<td width="35%"><input type="text" class="input fl" name="pwd" size="20" value="" tips="不修改则不填写" validate="required:true"/></td>
				<th width="15%">状态</th>
            ...
            <tr>
                <th width="15%">配送范围</th>
                <td>
                    <input type="hidden" id="delivery_range_polygon" name="delivery_range_polygon" value="">
                    <select id="delivery_range_type" name="delivery_range_type" >
                    <option value="0" selected>半径范围</option>
                    <option value="1">自定义范围</option>
                    </select>
                </td>
                <th width="15%" class="range">半径距离</th>
                <td class="range">
                    <input type="text" class="input fl" name="range" size="20" validate="required:true" value="5"/>公里
                </td>
			</tr>
			<tr>
    ...
    <if condition="$config['map_config'] eq 'google' AND $config['google_map_ak']">
        ...
        <script type="text/javascript" src="{pigcms{$static_path}js/map_google.js?t=111"></script>
        <else />
	...
	<script type="text/javascript" src="{pigcms{$static_path}js/map.js?t=11"></script>
    </if>
