		...
		<div class="mainbox">
			<div id="nav" class="mainnav_title">
				<ul>
					...
					<a href="javascript:void(0);" class="on">配送员评论列表</a>|
			...
			<table class="search_table" width="100%">
				<tr>
					<td>
                        <form action="{pigcms{:U('Deliver/reply')}" method="get" id="searchSubmit">
                            ...
                            <input type="hidden" name="a" value="reply"/>
    							...
    							<a href="{pigcms{:U('Deliver/exportComment', array('day' => $day, 'province_idss'=> $_GET['province_idss'], 'city_idss'=> $_GET['city_idss'], 'area_id'=> $_GET['area_id'], 'keyword'=> $keyword, 'period' => $period, 'nameType' => $_GET['nameType']))}" class="button" style="float:right;margin-right: 10px;">导出订单</a>
                        </form>
							...
							<if condition="is_array($supply_info)">
								<volist name="supply_info"  id="vo">
									<tr class="<if condition="$i%2 eq 0">odd<else/>even</if>">
										...
										<td width="30">{pigcms{$vo.username}<br/>{pigcms{$vo.userphone}</td>
                                        <if condition="$vo.comment_time gt 0">
                                        <td width="50">{pigcms{$vo.comment_time|date='Y-m-d H:i:s',###}</td>
                                        <else />
                                        <td width="50">--</td>
                                        </if>
										
										<td width="80">{pigcms{$vo.score}</td>
										<td width="80">{pigcms{$vo.comment}</td>
