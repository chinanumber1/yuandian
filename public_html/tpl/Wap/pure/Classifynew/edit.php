...
<body>
	...
	<div class="page__bd">
		<form action="{pigcms{:U('Classify/classify_modify',array('cid'=>$cid))}" method="post" id="form" enctype="multipart/form-data">
			...
			<div class="weui-cells__title" style="margin-bottom:1em;">
				【提醒】请认真全面填写信息，以便大家查阅。<br/>
				【声明】发布的所有信息，平台只负责发布、展示，与平台本身无关，不承担任何责任。<br/>
				【备注】<span style="color:red;">图片非必传项，无图片也可直接发布帖子</span>
			</div>
			<div class="weui-cells weui-cells_form">
				<volist name="catfield" id="vv" key="kk">
							...
							<div class="weui-cell__bd">
								<volist name='classify_userinput_info["content"]' id="userinput_info">
									<if condition='($userinput_info["type"] eq $vv["type"]) AND ($userinput_info["tn"] eq $vv["name"])'>
										<input name="input[{pigcms{$kk}][vv]" class="weui-input" type="<php>if($vv['inarr']==1){echo 'number';}else{echo 'text';}</php>" value="{pigcms{$userinput_info['vv']}" <php>if($vv['inarr']==1)echo 'onkeyup="value=clearNoNum(this.value)"';</php>/>
					...
					<elseif condition="$vv['type'] eq 2"/>
						<div class="weui-cell weui-cell_select weui-cell_select-after">
							...
							<div class="weui-cell__hd"><label class="weui-label">{pigcms{$vv['name']}</label></div>
							<div class="weui-cell__bd">
								<php>$selectedValue = '';</php>
								<volist name='classify_userinput_info["content"]' id="userinput_info">
									<if condition='($userinput_info["type"] eq $vv["type"]) AND ($userinput_info["tn"] eq $vv["name"])'>
										<php>$selectedValue = trim($userinput_info['vv']);</php>
									</if>
								</volist>
								<select class="weui-select" name="input[{pigcms{$kk}][vv]">
									<volist name="vv['opt']" id="opt">
										<option value="{pigcms{$opt}" <if condition="$opt eq $selectedValue">selected="selected"</if>>{pigcms{$opt}</option>
									</volist>
								</select>
						...
						<div class="weui-cell">
							...
							<div class="weui-cell__bd">
								<div class="post-tags cl" id="post-typeid" style="padding:5px 0px;">
									<php>$selectedValue = array();</php>
									<volist name='classify_userinput_info["content"]' id="userinput_info">
										<if condition='($userinput_info["type"] eq $vv["type"]) AND ($userinput_info["tn"] eq $vv["name"])'>
											<php>$selectedValue = $userinput_info['vv'];</php>
										</if>
									</volist>
									<volist name="vv['opt']" id="opt">
										<a class="weui-btn weui-btn_mini weui-btn_default <if condition="in_array($opt,$selectedValue)">tag-on</if>" href="javascript:;" onclick="return setTypeidNew(this);">{pigcms{$opt}</a>
										<input class="none" name="input[{pigcms{$kk}][vv][]" type="checkbox" value="{pigcms{$opt}" <if condition="in_array($opt,$selectedValue)">checked="checked"</if>/>
									</volist>
								</div>
								<input name="input[{pigcms{$kk}][tn]"  value="{pigcms{$vv['name']}"  type="hidden" />
								<input name="input[{pigcms{$kk}][input]"  value="{pigcms{$vv['input']}"  type="hidden" />
					...
					<elseif condition="$vv['type'] eq 5"/>
						<div class="weui-cell">
							...
							<div class="weui-cell__bd">
								<volist name='classify_userinput_info["content"]' id="userinput_info">
									<if condition='($userinput_info["type"] eq $vv["type"]) AND ($userinput_info["tn"] eq $vv["name"])'>
										<textarea name="input[{pigcms{$kk}][vv]" class="weui-textarea" rows="3">{pigcms{$userinput_info['vv']}</textarea>
										<input name="input[{pigcms{$kk}][tn]"  value="{pigcms{$userinput_info['tn']}"  type="hidden" />
										<input name="input[{pigcms{$kk}][input]"  value="{pigcms{$userinput_info['input']}"  type="hidden" />
										<input name="input[{pigcms{$kk}][iswrite]"  value="{pigcms{$userinput_info['iswrite']}"  type="hidden" />
										<input name="input[{pigcms{$kk}][isfilter]"  value="{pigcms{$userinput_info['isfilter']}"  type="hidden" />
                ...
                <div class="weui-cell">
					<div class="weui-cell__bd">
						<textarea name="description" class="weui-textarea" placeholder="请填写信息的内容，越详细越好" rows="4">{pigcms{$classify_userinput_info.description}</textarea>
					</div>
				</div>
				<div class="weui-cell">
					<div class="weui-cell__bd">
								...
								<ul class="weui-uploader__files" id="uploaderFiles">
									<if condition='$classify_userinput_info["imgs"]'>
										<volist name='classify_userinput_info["imgs"]' id='img_info'>
											<li class="weui-uploader__file weui-uploader__file_status" style="background-image:url({pigcms{$img_info})">
			...
			<div class="weui-cells weui-cells_form">
				<div class="weui-cell">
					...
					<div class="weui-cell__bd">
						<input name="lxname" class="weui-input" type="text" placeholder="请填写联系人姓名" value="{pigcms{$classify_userinput_info.lxname}"/>
					</div>
				</div>
				<div class="weui-cell">
					<div class="weui-cell__hd">
					...
					<div class="weui-cell__bd">
						<input name="lxtel" class="weui-input" type="tel" placeholder="请填写联系人电话号码" value="{pigcms{$classify_userinput_info.lxtel}"/>
					</div>
				</div>
			</div>
