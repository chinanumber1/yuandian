...
<meta charset="utf-8">
<title><if condition="$_GET['all_mer'] eq 1">全部商家优惠券<else />{pigcms{$now_merchant.name}的优惠券</if></title>
                        ...
                        <ul class="end_ul">
                            <li>
								...
								<volist name="coupon_list[$key]" id="vv">
                                    ...
                                    <dd >
                                        <a href="javascript:void(0);" class="clr" data-type="{pigcms{$vv.coupon_id}" data-cate_name="{pigcms{$vv.cate_name}" data-wxcardid="{pigcms{$vv.wx_cardid}" data-tickettime="{pigcms{$vv.wx_ticket_addtime}" data-sign="{pigcms{$vv.cardsign}" data-status="{pigcms{$vv.status}" data-allow_new="{pigcms{$vv.allow_new}" data-can_get_num="{pigcms{$vv.can_get_num}">
                                            <div class="left">
                                                <div class="top">
                                                    ...
                                                    <div class="p60">
                                                        ...
                                                        <p>· 满<em>{pigcms{$vv.order_money|floatval}</em>元使用</p>
                                                        <p style="color:black;">【商家】{pigcms{$vv.merchant_name}</p> 
                                                    </div>    
                                                </div>
                                                <div class="end">
...
<script type="text/javascript">
	...
	function ajax_had_pull(tmp){
		$.ajax({
			...
			type: 'POST',
			dataType: 'json',
			data: {coupon_id:$(tmp).data('type')},
			success:function(data){
				
				if(data.error_code){
					if($(tmp).data('can_get_num')<=0){
