...
$(function(){
	...
	$(document).on('click','.showMoreGoods',function(){
		if(flag_click){	
			$(this).parents('ul').find('li').show();		
			$(this).addClass('hideMoreGoods');
			flag_click =false;
		}else{
			$(this).parents('ul').find('li:gt(1)').hide();	
			//$('.goodList li:gt(2)').hide();
			$(this).parent().show()
			$(this).removeClass('hideMoreGoods');
			flag_click = true;
		}
		return false;
	});
	...
	pageLoadTip(103);
	getUserLocation({useHistory:false,okFunction:'getList'}); 
	//getList(true);
});
...
function getList(more){
	...
	if($.cookie('userLocationLong')==''){
		getUserLocation({useHistory:false,okFunction:'getList'}); 
		//getSearchList();
		return false;
	}else{
		user_long = $.cookie('userLocationLong');
			go_url+="&user_lat="+$.cookie('userLocationLat')+'&user_long='+user_long;
	}
	//console.log($.cookie);
	now_page += 1;
	go_url += "&page="+now_page;
	$.post(go_url,function(data){
		var result = data.result;
		if(data.errorCode){
			alert(data.errorMsg)
		}else{
			if(result.store_list.length > 0){
				hasMorePage = result.has_more ;
				laytpl($('#groupListBoxTpl').html()).render(result.store_list, function(html){
					if(more){
						if(hasMorePage){
							$("#pullUp").removeClass('noMore loading').show();
						}
