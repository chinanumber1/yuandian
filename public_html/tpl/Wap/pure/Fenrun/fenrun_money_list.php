...
<html lang="zh-CN">
 ...
 <body>
        <section class="wallet">
            <div class="operation">
               <div class="name">分润钱包（元）</div>
                <div class="num">{pigcms{$now_user.fenrun_money|floatval}</div>
                <div class="turn">转余额</div>
            </div>
            <div class="record"> 
				<div class="h2">分润记录</div>	
		...
		<script>
			...
			$('.turn').on('click',function(){
				location.href =	"{pigcms{:U('fenrun_to_balance')}";
			});
						...
						if(result){
							...
							if(ride_list){
								...
								for(var x=0;x<ride_list_length;x++){
									...
									rideList	+=	'	<h2>'+ride_list[x].fenrun_money+'</h2>';
									rideList	+=	'	<p class="explain">'+ride_list[x].des+'</p>';
									rideList	+=	'	<p>'+ride_list[x].add_time+'</p>';
									rideList	+=	'</div>';
									if(ride_list[x].type == 2){
										rideList	+=	'	<div class="fr c5b"> -'+ride_list[x].fenrun_money+'</div>';
									}else{
										rideList	+=	'	<div class="fr cf1"> +'+ride_list[x].fenrun_money+'</div>';
									}
									rideList	+=	'</li>';
								}
