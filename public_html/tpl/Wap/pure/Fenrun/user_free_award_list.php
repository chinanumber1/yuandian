...
<html lang="zh-CN">
 ...
 <body>
        <section class="wallet">
            <div class="operation orange">
              <div class="name">可用佣金（元）</div>
                <div class="num">{pigcms{$now_user.free_award_money|floatval}</div>
                <div class="turn">转余额</div>
            </div>
            <div class="record"> 
				 <div class="h2">佣金记录</div>
		...
		<script>
			...
			$('.turn').on('click',function(){
				location.href =	"{pigcms{:U('award_to_balance')}";
			});
			...
			function list(){
				$.ajax({
					type : "post",
					url : "{pigcms{:U('ajax_user_free_award_list')}",
					dataType : "json",
					data:{
						page	:	page,
					},
