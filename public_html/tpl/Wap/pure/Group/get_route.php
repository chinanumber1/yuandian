    ...
    <if condition="$config['map_config'] eq 'google' AND $config['google_map_ak']">
        <script>
            ...
            function realResult(result){
                ...
                $('#frame_src').attr('src','https://www.google.com/maps/embed/v1/directions?key={pigcms{$config.google_map_ak}&origin='+result.y+','+result.x+'&destination={pigcms{$now_store.lat},{pigcms{$now_store.long}');
                motify.clearLog();
            }
        </script>
        <else />
