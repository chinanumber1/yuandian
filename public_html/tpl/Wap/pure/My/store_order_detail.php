...
<html lang="zh-CN">
    ...
    <body>
		<if condition="$is_wexin_browser AND $config['open_share_lottery'] eq 1 AND !empty($lottery) AND $lottery_info.status neq 1 AND $order.paid  eq 1 AND $order_details.share_status eq 0 AND !empty($share_coupon)">
			<div class="t_remind <if condition="$weixin_browser_type eq 'android'">az</if>">
				<p>{pigcms{$lottery['detail_msg']}</p>
			</div>
		</if>
		
		<div class="t_remind <if condition="$weixin_browser_type eq 'android'">az</if>" style="min_height:0px;height:0px;display:none">
			<p>{pigcms{$config.share_coupon_notice}</p>
		</div>
		<!--<if condition="$is_wexin_browser AND $config.share_coupon eq 1" >-->
		...
		<elseif condition="$config.open_share_lottery eq 0 AND $order.paid  eq 1 AND $config.share_coupon eq 1 AND $is_wexin_browser AND $is_wexin_browser AND $config.get_coupon_must_share eq 0 AND !empty($share_coupon)" />
		 ...
		 <if condition="$config.open_share_lottery eq 0 AND $order.paid  eq 1 AND $is_wexin_browser AND ($order['share_status'] eq 0 OR $config.get_coupon_must_share eq 0)  AND $config.share_coupon eq 1 AND $config.share_coupon_num gt 0  AND !empty($share_coupon) ">
		  ...
		  <if condition="$config.open_share_lottery eq 0 AND $order.paid  eq 1  AND $is_wexin_browser AND ($order['share_status'] eq 1 OR $config['get_coupon_must_share'] eq 0) AND $config.share_coupon eq 1 AND $config.share_coupon_num gt 0 AND $order.show_lottery_first eq 0 AND !empty($share_coupon)">
	...
	<script>
			...
			<if condition="$config.open_share_lottery eq 0 AND $order.paid  eq 1 AND $is_wexin_browser AND  $config.share_coupon eq 1 AND $order['share_status'] eq 0 AND $config.get_coupon_must_share eq 0">
					$.post('{pigcms{$config.site_url}{pigcms{:U('My/ajax_share_friend')}', {order_type:'store',order_id: {pigcms{$order['order_id']}}, function(data, textStatus, xhr) {
						if(!data.status){
							//layer.open({content:data.info,btn: ['确定']});
						}
			...
			<if condition="$config.open_share_lottery eq 1 AND $order.paid  eq 1 AND $is_wexin_browser  AND $order['share_status'] eq 0 AND $config.lottery_must_share eq 0">
					$.post('{pigcms{$config.site_url}{pigcms{:U('My/ajax_share_friend')}', {order_type:'store',order_id: {pigcms{$order['order_id']}}, function(data, textStatus, xhr) {
						if(data.status){
							$('#float-open').show(); 
						}
