...
<html lang="zh-CN">
 ...
 <body>
        <section class="wallet">
            <div class="operation integral">
                <div class="name">当前积分</div>
                <div class="num">{pigcms{$now_user.score_count|floatval}</div>
            </div>
            <div class="record">
                <div class="h2">积分记录</div>
                <ul class="bw">
						...
						if(result){
							...
							if(ride_list){
								...
								for(var x=0;x<ride_list_length;x++){
									...
									rideList	+=	'	<h2>'+ride_list[x].desc+'</h2>';
									rideList	+=	'	<p>'+ride_list[x].time_s+'</p>';
									rideList	+=	'</div>';
									if(ride_list[x].type == 2||ride_list[x].type == 3){
										rideList	+=	'	<div class="fr c5b"> -'+ride_list[x].score+'</div>';
									}else{
										rideList	+=	'	<div class="fr cf1"> +'+ride_list[x].score+'</div>';
									}
									rideList	+=	'</li>';
								}
