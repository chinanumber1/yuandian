            ...
            <li>
                ...
                <div class="p18">
                    <div class="con">
                        ...
                        <elseif condition="$vo['status'] eq 4"/> <h2>配送员到店</h2> <p>配送员:<if condition="$vo['name']"><strong style="color:red">【{pigcms{$vo.name}】</strong></if> <a class="tel-btn" href="tel:{pigcms{$vo.phone}">{pigcms{$vo.phone}</a>已到店，准备配送，请耐心等待</p>
                        ...
                        <elseif condition="$vo['status'] eq 9"/> <h2>已完成退款</h2> <p><php> if (empty($vo['note'])) { </php>您已完成退款<php> } else { </php>{pigcms{$vo.note}<php>}</php></p>
                        <elseif condition="$vo['status'] eq 10"/> <h2>已取消订单</h2> <p><php> if (empty($vo['note'])) { </php>您已经取消订单<php> } else { </php>{pigcms{$vo.note}<php>}</php></p>
                        ...
                        <elseif condition="$vo['status'] eq 31"/> <h2>配送员放弃配送</h2> <p>{pigcms{$vo.note}</p>
                        <elseif condition="$vo['status'] eq 34"/> <h2>订单已被取消</h2> <p>已为您退款！</p>
                        </if>
                        <if condition="$i eq 1">
                        <div class="map clr">
...
<if condition="$config['map_config'] eq 'google' AND $config['google_map_ak']">
    ...
    <script type="text/javascript">
        var status = '{pigcms{$order['order_status']}', isShowMap = '{pigcms{$isShowMap}';
