...
<html lang="zh-CN">
    ...
    <body>
        ...
        <section class="public">
            <a class="return link-url" id="goBackUrl" href="{pigcms{:U('My/shop_order_list')}"></a>
		...
		<if condition="$config.open_share_lottery eq 1 AND  $order_details.paid  eq 1 AND $lottery_info.status neq 1">
		...
		<elseif condition="$config.open_share_lottery eq 0  AND $config.share_coupon eq 1 AND  $config.get_coupon_must_share eq 0 AND $order_details.paid  eq 1 AND !empty($share_coupon) AND $order_details.status neq 4" />
		 ...
		 <if condition="$config.open_share_lottery eq 0 AND $order_details.paid  eq 1 AND (($order_details['share_status'] eq 0 AND $config.get_coupon_must_share eq 1) OR $config.get_coupon_must_share eq 0) AND $config.share_coupon eq 1 AND $config.share_coupon_num gt 0 AND $order_details.status neq 4 AND !empty($share_coupon) AND !empty($share_coupon)">
		  ...
		  <if condition="$config.open_share_lottery eq 0 AND $order_details.paid  eq 1 AND  ($order_details['share_status'] eq 1 OR $config['get_coupon_must_share'] eq 0)  AND $config.share_coupon eq 1 AND $config.share_coupon_num gt 0 AND $share_coupon_info.get_num eq 0 AND $order_details.status neq 4 AND !empty($share_coupon)">
            ...
            <div class="coupon_share Coupon afters" >
                ...
                <p style="text-align: center;">待领取</p>
	...
	<script>
			...
			function call_back_now(){
				<if condition="$config.open_share_lottery eq 0 AND $config.share_coupon eq 1 AND $order_details.paid  eq 1">
					...
					$.post('{pigcms{$config.site_url}{pigcms{:U('My/ajax_share_friend')}', {order_type:'shop',order_id: {pigcms{$order_details['order_id']}}, function(data) {
						// if(!data.status){
			...
			<if condition="$config.open_share_lottery eq 0 AND  $order_details.paid  eq 1  AND $config.share_coupon eq 1 AND $order_details['share_status'] eq 0 AND $config.get_coupon_must_share eq 0">
				$.post('{pigcms{$config.site_url}{pigcms{:U('My/ajax_share_friend')}', {order_type:'shop',order_id: {pigcms{$order_details['order_id']}}, function(data, textStatus, xhr) {
					// layer.open({content:data.info,btn: ['确定'],
					 // yes: function(index){
						  // location.reload();
			...
			<if condition="$config.open_share_lottery eq 1 AND  $order_details.paid  eq 1  AND $order['share_status'] eq 0 AND $config.lottery_must_share eq 0">
			...
			$('#share_btn').click(function(){
				...
				"<if condition="$config['share_coupon'] eq 1 AND ($config['open_share_lottery'] eq 0 OR empty($lottery))">{pigcms{$config.share_coupon_title}<else />{pigcms{$config.shop_alias_name|default="快店"} - {pigcms{$config.site_name}</if>",
				"<if condition="$config['share_coupon'] eq 1 AND ($config['open_share_lottery'] eq 0 OR empty($lottery))">{pigcms{$config.share_coupon_num}张优惠劵，快来抢啊！<else />{pigcms{$config.seo_description}</if>","<if condition="$config['wechat_share_img']">{pigcms{$config.wechat_share_img}<else />{pigcms{$config.site_logo}</if>",
				"<if condition="$config['share_coupon'] eq 1 AND ($config['open_share_lottery'] eq 0 OR empty($lottery))">{pigcms{$config.site_url}{pigcms{:U('Share_lottery/share_coupon',array('order_id'=>$order_details['order_id'],'type'=>'shop'))}<else />{pigcms{$config.site_url}{pigcms{:U('Shop/index')}&shop-id={pigcms{$store.store_id}</if>");
			})
