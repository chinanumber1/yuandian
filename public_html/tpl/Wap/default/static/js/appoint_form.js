...
var mapObjDom = null;
$(function(){
	if(change_select_store == '' && $('#store_id').val()>0 && (merchant_workers_id == '' || merchant_workers_id == 0)){
...
function select_store(){
	...
	$.post(ajaxWorkUrl,{'merchant_store_id':store_id,'appoint_id':appoint_id},function(data){
		...
		if(data.status){
			...
			//if(flag){
			var work_info = data.worker_list[0];
			console.log('技师信息-------', work_info);
			if (work_info.is_reward == 1) {
                str +='<div class="yxc-attr-list detail"><div><img src="' + data.worker_list[0]["avatar_path"] + '" class="detail-tou" /></div><div class="detail-intro"><p>'+data.worker_list[0]["name"]+'</p><p>'+data.worker_list[0]["desc"]+'</p><p>共服务'+data.worker_list[0]["finish_count"]+'次</p></div><div class="detail-right"><a href="'+workerUrl+'"><img src="' + static_path +'images/detail_gh.png" /></a></div></div><div class="yxc-space"></div>';
            } else if (work_info.is_reward == 2) {
                str +='<div class="yxc-attr-list detail" style="padding-bottom:0.6rem;">' +
                    '<div style="position:relative;">' +
                    '<img src="' + data.worker_list[0]["avatar_path"] + '" class="detail-tou"/>' +
                    '<div class="tip_reward" onclick="worker_reward_pay(' + work_info.merchant_worker_id + ')">' +
                    '<div style="margin:30% auto;font-size:0.2rem;width:62%;text-align:center;">打赏后查看清晰图</div>' +
                    '</div>' +
                    '<div style="width: 94%;font-size: 0.8rem;background-color: #46d2e6;color: white;text-align: center;border-radius: 5%;float: none;padding: 0;" onclick="worker_reward_pay(' + work_info.merchant_worker_id + ')">去打赏</div>' +
                    '</div>' +
                    '<div class="detail-intro"><p>'+data.worker_list[0]["name"]+'</p><p>'+data.worker_list[0]["desc"]+'</p><p>共服务'+data.worker_list[0]["finish_count"]+'次</p></div><div class="detail-right"><a href="'+workerUrl+'"><img src="' + static_path +'images/detail_gh.png" /></a></div></div><div class="yxc-space"></div>';
