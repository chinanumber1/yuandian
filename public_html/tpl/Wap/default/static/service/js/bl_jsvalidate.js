...
PublishFormCheck({
    ...
    "validateOption":{
       "user_name":{
          ...
          'priceCheck':{"msg":"能输入正确的价格"},//价格 验证

          "numberCheck":{"range":[5,10,"数字个数必须大于5小于10"]},//数字区间
          "numberCheck":{"max":[10,"最大不能超过10个数字"]},//最大数字
          "numberCheck":{"min":[10,"最小不能少于10个数字"]},//最小数字
...
var PublishFormCheck=function(args){
    ...
    if(args.errorNoPosition){
        $(args.formId).attr("errorNoPosition",'true');
    }
...
var formSubmitCheck=function(args){
    ...
    if(!allSuccess){
        ...
        /*定位第一个 报错 start*/
        if($(args.formId).attr('errorNoPosition')!="true"){
    ...
    if (args.rangelength != undefined) {
        ...
        if(!(vallength>=args.rangelength[0]&&vallength<=args.rangelength[1])){
            if(args.rangelength.length==3){
                ...
                if(args.rangelength.length==4){
                    if(!(vallength>args.rangelength[0])){
                        setErrormsg({"parentEle":args.parentEle,"msg":args.rangelength[2]});
                    }else{
                        setErrormsg({"parentEle":args.parentEle,"msg":args.rangelength[3]});
                    }
                }else{
...
/*价格 验证*/
/*args:{"parentEle":,"curOjb":,"msg"}*/
var priceCheck=function(args){
    var checkState= false;
    var quanjiaoRegex=/[^\x00-\xff]/g;//全角
    var priceRegex=/^(0|[1-9][0-9]{0,7})(\.[0-9]{1,2})?$/;  //不包括0
    var val=$(args.curOjb).val();
    if(quanjiaoRegex.test(val)){
        ...
        if(!(priceRegex.test(val))){
            if (args.msg != undefined) {
                ...
                setErrormsg({"parentEle":args.parentEle,"msg":errorPrice});
            }
            checkState=true;
        }else{
            if(val==0){
...
var numberCheck=function(args){
    var checkState= false;
    var curOjbTagName=$(args.curOjb)[0].tagName;
    var curOjbType;
    var valNumber=$(args.curOjb).val();
    if (args.range != undefined) {
        /*数字区间*/
        if(!(valNumber>=args.range[0]&&valNumber<=args.range[1])){
            if(args.range.length==3){
                setErrormsg({"parentEle":args.parentEle,"msg":args.range[2]});
            }else{
                if(args.range.length==4){
                    if(!(valNumber>=args.range[0])){
                        setErrormsg({"parentEle":args.parentEle,"msg":args.range[2]});
                    }else{
                        setErrormsg({"parentEle":args.parentEle,"msg":args.range[3]});
                    }
                }else{
                    var curmsg='请输入大于'+args.range[0]+' 小于'+args.range[1]+'的数字';
                    setErrormsg({"parentEle":args.parentEle,"msg":curmsg});
                }
            }
            checkState=true;
        ...
        if (args.min != undefined) {
            if(valNumber<args.min[0]){
                if(args.min.length==2){
                    var curmsg=args.min[1];
                    setErrormsg({"parentEle":args.parentEle,"msg":curmsg});

                }else{
                    var curmsg='请输入大于'+args.min[0]+'的数字';
                    setErrormsg({"parentEle":args.parentEle,"msg":curmsg});
                }
                checkState=true;
            }
        ...
        if (args.max != undefined) {
            if(valNumber>args.max[0]){
                if($(args.curOjb).attr("type")=="checkbox"){
                    $(args.curOjb).prop("checked",false);
                }

                if(args.max.length==2){
                    var curmsg=args.max[1];
                    setErrormsg({"parentEle":args.parentEle,"msg":curmsg});
                }else{
                    var curmsg='请输入小于'+args.max[0]+'的数字';
                    setErrormsg({"parentEle":args.parentEle,"msg":curmsg});
                }
                checkState=true;
...
var ajaxCheck = function(args){
    ...
    if(args.ajaxFun!=undefined&&$(args.curOjb).attr("ajaxCheck")!="submit"){
        ...
        $(args.curOjb).attr("ajaxComplete","false");
        $.ajax({
            url: args_url,
            type: args_type,
            ...
            data:dataobj,
            success: function(data){
                $(args.curOjb).attr("ajaxComplete","true");
                var callbackAgrs={};
                if(args_success!=undefined){
                    ...
                    if(callbackAgrs!=undefined){
                        ...
                        if($(args.curOjb).closest("form").hasClass("unblur-error")){
                            ...
                            var curIndex=$(args.curOjb).closest('[checksuccess]').index();
