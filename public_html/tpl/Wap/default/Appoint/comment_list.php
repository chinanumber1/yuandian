...
<body id="index" data-com="pagecommon">
            ...
            <div id="nav-dropdown" class="nav-dropdown">
                <ul>
                    <li><a class="react" href="{pigcms{:U('Home/index')}"><i class="text-icon">⟰</i>
                        ...
                        <space></space>我的</a>
                    </li><li><a class="react" href="{pigcms{:U('Search/index',array('type'=>'appoint'))}"><i class="text-icon">⌕</i>
                        <space></space>搜索</a>
                </li></ul>
            </div>
        </header>
			    ...
			    <dd>
			        <dl>
						...
						<span class="stars">
							...
							<for start="1" end="6">
								<if condition="$score_mean egt $i">
									<i class="text-icon icon-star"></i>
								<elseif condition="$score_mean gt $i-1 AND $score_mean lt $i"/>
							...
							<em class="star-text">{pigcms{$score_mean}</em></span></span></dt>
							...
							<dd class="dd-padding">
								<div class="feedbackCard">
									...
									<div class="score">
										<span class="stars">
											<for start="1" end="6">
												...
												<if condition="$vo['score'] egt $i">
													<i class="text-icon icon-star"></i>
												<elseif condition="$vo['score'] gt $i-1 AND $vo['score'] lt $i"/>
													<i class="text-icon icon-star-gray">
													<i class="text-icon icon-star-half"></i>
													</i>
												<else/>
										...
										<weak class="time"><if condition="$vo.anonymous eq 0">{pigcms{$vo.nickname}<else />匿名用户</if></b>&nbsp;&nbsp;{pigcms{$vo.add_time|date="Y-m-d H:i:s",###}</weak>
									</div>
									<div class="comment">
										<p>{pigcms{$vo.content}</p>
									</div>
									<if condition="$vo['comment_img']">
										<div class="pics view_album" data-pics="<volist name="vo['comment_img']" id="voo">{pigcms{$voo}<if condition="count($vo['comment_img']) gt $i">,</if></volist>">
											<volist name="vo['comment_img']" id="voo">
												<span class="pic-container imgbox" style="background:none;"><img src="{pigcms{$voo}" style="width:100%;"/></span>&nbsp;
											</volist>
										</div>
									</if>
								</div>
		...
		<script>var static_path="{pigcms{$static_path}";var collect_url="{pigcms{:U('Collect/collect')}";var appoint_id="{pigcms{$now_appoint.appoint_id}";</script>
