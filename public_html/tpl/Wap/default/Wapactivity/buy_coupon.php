...
<body>
	...
	<script>
		var buyType = <if condition="$now_activity['money']">1<else/>2</if>,buyValue = <if condition="$now_activity['money']">{pigcms{$now_activity.money}<else/>{pigcms{$now_activity.mer_score}</if>,buySysScore = {pigcms{:floatval($now_activity['mer_score']/$config['activity_score_scale'])};
	</script>
	<script>
		$(function(){
