    ...
    <div id="fis_elm__2">
        <div id="address-widget-map" class="address-widget-map">
            <div class="address-map-nav">
                ...
                <div class="center-title" style="margin-top:8px"> <i class="icon-location" data-node="icon"></i>
	...
	$('#addressShow').delegate("li","click",function(){ 
		...
		$.get(cityMatchingUrl, {'lng':lng,'lat':lat}, function(data){
			if(data.error == 1){
				...
				location.href = "{pigcms{:U('Service/edit_service_publish',array('paid'=>$_GET['paid']))}&sname="+sname+"&lng="+lng+"&lat="+lat+"&area_id="+data.area_id+"&area_name="+data.area_name;
			}else{
				layer.open({
					content: data.msg
					,btn: ['确定']
