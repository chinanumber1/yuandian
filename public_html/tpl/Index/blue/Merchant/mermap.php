...
<include file="Public:footer"/>
		...
		<script type="text/javascript">
            if(typeof is_google_map != "undefined"){
                var lng = {pigcms{$nowstore['long']};
                var lat = {pigcms{$nowstore['lat']};
                pos = {lng:parseFloat(lng),lat:parseFloat(lat)};
                var map = new google.maps.Map(document.getElementById('container'), {
                    mapTypeControl:false,
                    zoom: 18,
                    center: pos
                });
                var contentString = "<b>{pigcms{$nowstore['name']}</b><br/><b>地址：</b>{pigcms{$vo.area_name}{pigcms{$nowstore['adress']}<br/><b>电话：</b>{pigcms{$nowstore['phone']}<br/><b>线路：</b><a href='https://www.google.com/maps/place/{pigcms{$nowstore['adress']|urlencode=###}' target='_blank'>公交/驾车路线查询»</a>";
                ...
                $.each($('.search-path'),function(i,item){
                    $(item).attr({'href':'https://www.google.com/maps/place/'+encodeURIComponent($(item).attr('shop_name')),'target':'_blank'});
                });
            }else{
