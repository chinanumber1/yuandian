...
// 公共弹出窗体
// title：窗体标题，url：加载远程url，callback点击确定按钮触发的回调函数
function ShowWindow(title,url,callback){
	var html = '<div class="modal fade" id="_public_window" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">\
				  <div class="modal-dialog" role="document">\
				    <div class="modal-content">\
				      ...
				      <div class="modal-body" id="_public_window_body"></div>\
				      <div class="modal-footer">\
				        <button type="button" class="btn btn-primary" id="fire_ok">确定</button>\
				      </div>\
				    </div>\
				  </div>\
				</div>';
	...
	$('#_public_window').on('hidden.bs.modal', function (e) {
		if(callback!=undefined){
			callback();
		}
		$('#_public_window').remove();
	});

	$('#fire_ok').on('click',function(){
		...
		$('#_public_window').modal('hide');
	});

	$('#_public_window_body').load(url,function(response){
		$('#_public_window').modal('show');
	});
}