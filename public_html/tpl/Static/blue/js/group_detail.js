$(function(){
	...
	if(typeof(is_google_map)!="undefined"){
		...
		var point = {lng:parseFloat(store_long),lat:parseFloat(store_lat)};
	...
	$('.J-piclist-wrapper li a').live('click',function(){
		...
		imgObj.onload = function(){
			...
			if(imgWidth > ($(window).width()*0.8)){
				...
				var imgHeight = imgHeight*imgWidth/imgObj.width;
			}
			if(imgHeight > ($(window).height()*0.8)){
				var imgHeight = $(window).height()*0.8;
				var imgWidth = imgWidth*imgHeight/imgObj.height;
			}
			...
			window.art.dialog({
				...
				left: '50%',
				top: '38.2%',
				content:'<a href="'+big_src+'" target="_blank" title="新窗口打开查看原图"><img src="'+m_src+'" alt="大图" style="max-width:'+imgWidth+'px;max-height:'+imgHeight+'px;"/></a>',
				close: null
			});
		};
		imgObj.src = m_src;
