				...
				if(input_dom.attr('data-max') != '0' && input_Val >= parseInt(input_dom.attr('data-max'))){
					...
					if(extra_price>0){						
						...
						if(open_extra_price==1&& extra_price>0){
							...
							$('#deal-buy-total-t,#levelofftotal').html(((input_Val+1)*(finalprice*10000))/10000+'+'+extra_price*(input_Val+1)+extra_price_name);
						}else{
							...
							total_money = ((input_Val+1)*(finalprice*10000))/10000;
							$('#J-deal-buy-total').html(total_money);
						
							if(total_money>=full_money && full_money>0){
								$('.express_fee').html('0 元');
								$('#deal-buy-total-t,#levelofftotal').html(total_money+'元');
							}else{
								express_fee = express_fee_tmp;
								if( full_money>0){
									$('.express_fee').html( express_fee+'元'+'('+'满'+full_money+'包邮 '+')');
								}
		...
		if(!/^-?(?:\d+|\d{1,3}(?:,\d{3})+)(?:\.\d+)?$/.test($(this).val())){
			...
			if(input_dom.attr('data-max') != '0' && input_Val >= parseInt(input_dom.attr('data-max'))){
				...
				if(open_extra_price && extra_price >0){
					...
					$('#J-deal-buy-total,#deal-buy-total-t').html((input_Val)*(group_price*10000)/10000+'+'+extra_price*(input_Val)+extra_price_name);
				}else{
					
						total_money = (input_Val)*(group_price*10000)/10000;
