		...
		<div class="page-content-area">
			<div class="row">
				<div class="col-xs-12" style="padding-left:0px;padding-right:0px;">
					<if condition="$_GET['page'] lt 2">
						<div class="widget-box">
							...
							<script type="text/javascript">
								...
								<else/>
									$('#behavior_chart').html('<h2 style="margin:150px 0 50px;">7天内粉丝行为统计</h2><p style="margin-bottom:150px;">暂无数据</p>');
								</if>
								
								<if condition="$man_count || $woman_count || $unsexman_count">
								...
								<else/>
									$('#sex_chart').html('<h2 style="margin:150px 0 50px;">粉丝性别比例图</h2><p style="margin-bottom:150px;">暂无数据</p>');
								</if>
							</script>
						</div>
					...
					<form action="{pigcms{:U('Customer/fans_list')}" method="get" style="float:left;margin-left:10px">
						<input type="hidden" name="c" value="Customer"/>
						<input type="hidden" name="a" value="fans_list"/>
						<select name="searchtype">
							<option value="nickname" <if condition="$_GET['searchtype'] eq 'nickname'">selected="selected"</if>>昵称</option>
							<option value="phone" <if condition="$_GET['searchtype'] eq 'phone'">selected="selected"</if>>手机</option>
						...
						&nbsp;&nbsp;&nbsp;  <a href="{pigcms{:U('Customer/fans_list', array('from_merchant' => $cl['from_merchant']))}">{pigcms{$from_type[$cl['from_merchant']]}({pigcms{$cl['count']})</if></a>
						</volist>
						 <button onclick="exports()">导出粉丝</button>
					</div>
...
<script type="text/javascript">
 ...
 var url = "{pigcms{$config.site_url}"
    var export_url = "{pigcms{:U('Customer/export')}"
</script>
