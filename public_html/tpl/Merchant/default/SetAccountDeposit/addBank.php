...
<div class="main-content">
    ...
    <div class="breadcrumbs" id="breadcrumbs">
        <ul class="breadcrumb">
			...
			<li class="active"><a href="{pigcms{:U('SetAccountDeposit/addBank')}">添加银行卡</a></li>
        </ul>
    </div>
	<div class="page-content form-horizontal ">
        <div class="page-content-area">
            <div class="row">
                <div class="col-xs-12">
					<form class="form" method="post" action="{pigcms{:U('SetAccountDeposit/bind_card')}" target="_top" enctype="multipart/form-data">
						...
						<div class="tab-content card_new" id="tab1" >
							...
							<div class="form-group">
								<label class="tiplabel"><label><font color="red">*</font>真实姓名</label></label>
								<if condition="$deposit['type'] neq '3'">
							{pigcms{$deposit.company_info.legalName}
							<else />
							{pigcms{$deposit.realName}
							</if><span class="tip"></span>
								
							</div>
							
							<div class="form-group">
								<label class="tiplabel"><label><font color="red">*</font>证件类型</label></label>
								身份证<span class="tip"></span>
								
							</div>
							
							<div class="form-group">
								<label class="tiplabel"><label><font color="red">*</font>证件号码</label></label>
								<if condition="$deposit['type'] neq '3'">
						
						<php> echo substr($deposit['company_info']['legalIds'],0,4).'******'.substr($deposit['company_info']['legalIds'],-4);</php>
						<else />
							<php> echo substr($deposit['identityNo'],0,4).'******'.substr($deposit['identityNo'],-4);</php>
							</if><span class="tip"></span>
								
							</div>
							<div class="form-group">
								<label class="tiplabel"><label><font color="red">*</font>银行名称</label></label>
								<select name="unionCode" id="unionCode" class="px">
							...
							<div class="form-group">
								<label class="tiplabel"><label><font color="red">*</font>银行卡号</label></label>
								<input type="text" name="cardNo" id="cardNo" class="px" value="" style="width:210px;"/><span class="tip"></span>
								
							</div>
							
							<div class="form-group">
								<label class="tiplabel"><label><font color="red">*</font>手机号</label></label>
			...
			if($("input[name='phone']").val()==''){
				...
				if(countdown==60){
					$.ajax({
						...
						type: 'POST',
						dataType: 'json',
						data: {phone: $("input[name='phone']").val(),cardNo:$('#cardNo').val(),realName:'<if condition="$deposit['type'] neq '3'">{pigcms{$deposit.company_info.legalName}<else />{pigcms{$deposit.realName}</if>',identityNo:'<if condition="$deposit['type'] neq '3'">
						{pigcms{$deposit.company_info.legalIds}<else />{pigcms{$deposit.identityNo}</if>',unionCode:$('#unionCode').val()},
