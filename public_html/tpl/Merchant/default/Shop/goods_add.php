						...
						<div class="tab-content">
							<div id="basicinfo" class="tab-pane active">
								...
								<div class="form-group">
									<label class="col-sm-1"><label for="name">快速录入</label></label>
									<input class="col-sm-2" name="sysname" id="sysname" type="text" placeholder="扫商品条码录入"/> 
									<a class="btn btn-sm btn-success" onclick="addLink('sysname')" style="line-height: 1.2;margin-left: 10px">从商品库选择</a>
								</div>
								<div class="form-group">
								...
								<div class="form-group">
									...
									<input class="col-sm-1" size="20" name="cost_price" id="cost_price" type="text" value=""/>
									<span class="form_tips">进货价用户是看不到</span>
								</div>
								<div class="form-group">
									<label class="col-sm-1"><label for="price">商品现价</label></label>
									<input class="col-sm-1" size="20" name="price" id="price" type="text" value=""/>
                                ...
                                <div class="form-group">
                                    ...
                                    <select name="limit_type" id="limit_type">
                                        <option value="0" selected>每单限购</option>
                                        <option value="1">每个ID限购</option>
                                    </select>
                                </div>
                                <div class="form-group">
                                    <label class="col-sm-1"><label id="label_limit">每单限购</label></label>
                                    <input class="col-sm-1" size="20" name="max_num" id="max_num" type="text" value="0"/>
                                    <span class="form_tips" id="label_tips">每个订单该商品最多可订购的数量，0为不限制</span>
                                </div>
								<div class="form-group">
								...
								<div class="form-group">
									...
									<select name="is_discount" id="is_discount">
										<option value="1" selected="selected">参与折扣</option>
										<option value="0" >不参与折扣</option>
									</select>
								...
								<div class="topic_box">
									...
									<tbody>
										<tr>
											...
											<th style="display:none">原价</th><th>进价</th><th>现价</th><th>限时价</th><th>每单限购</th><th>库存</th>
										...
										<volist name="now_goods['list']" id="gl" key="num">
											<tr id="{pigcms{$gl['index']}">
												...
												<td><input type="text" class="txt" name="max_nums[]" value="{pigcms{$gl['max_num']}" style="width:80px;"></td>
...
$(document).ready(function(){
	...
	$('#sysname').keypress(function(e){
		...
		if(e.keyCode==13){
			...
			$.get('?g=Merchant&c=Shop&a=ajax_goods', {'number':$(this).val()}, function(response){
				if (response.error) {
					alert(response.msg);
				} else {
					$('#name').val(response.data.name);
					$('#number, #sysname').val(response.data.number);
					$('#unit').val(response.data.unit);
					$('#price').val(response.data.price);
					$('#cost_price').val(response.data.cost_price);
					$('#upload_pic_ul').html('<li class="upload_pic_li"><img src="' + response.data.image + '"/><input type="hidden" name="pic[]" value="' + response.data.imageTitle + '"/><br/><a href="#" onclick="deleteImage(\'' + response.data.imageTitle + '\',this);return false;">[ 删除 ]</a></li>');
				}
...
function searchGoods(number)
{
	$.get('?g=Merchant&c=Shop&a=ajax_goods', {'goods_id':number}, function(response){
		if (response.error) {
			alert(response.msg);
		} else {
			$('#name').val(response.data.name);
...
function addLink(){
	art.dialog.data('fun', searchGoods);
	art.dialog.open('?g=Merchant&c=Shop&a=sys_goods',{lock:true,title:'商品库商品',width:800,height:550,yesText:'关闭',background: '#000',opacity: 0.45});
}
</script>
