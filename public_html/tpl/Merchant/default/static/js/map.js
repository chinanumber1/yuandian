	...
	var setPoint = function(mk,b){
		...
		(new BMap.Geocoder()).getLocation(pt,function(rs){
			...
			if (b===true){
				if(addComp.province && typeof($('#choose_province')) != 'undefined'){
					$.each($('#choose_province option'),function(i,item){
						...
						if(text && addComp.province.indexOf(text)!=-1){
							...
							if(choose_province.find('option').eq(i).prop('selected') == false){
				...
				if(addComp.city && typeof($('#choose_city')) != 'undefined'){
					$.each($('#choose_city option'),function(i,item){
						...
						if(text && addComp.city.indexOf(text)!=-1){
							...
							if(choose_city.find('option').eq(i).prop('selected') == false){
				...
				if(addComp.district && typeof($('#choose_area')) != 'undefined'){
					$.each($('#choose_area option'),function(i,item){
						...
						if(text && addComp.district.indexOf(text)!=-1){
							...
							if(choose_area.find('option').eq(i).prop('selected') == false){
				...
				if(addComp.market && typeof($('#choose_market')) != 'undefined'){
					$.each($('#choose_market option'),function(i,item){
						var text = $(item).html();
						if(text && addComp.market.indexOf(text)!=-1){
							var choose_market = $('#choose_market');
							if(choose_market.find('option').eq(i).prop('selected') == false){	
								choose_market.find('option').eq(i).prop('selected',true);
								show_market(choose_market.find('option:selected').attr('value'),choose_market.find('option:selected').html(),1);
							}
							return false;
						}
					});
