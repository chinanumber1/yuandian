...
function show_province(){
	$.post(choose_province,function(result){
		...
		if(result.error == 0){
			...
			area_dom += '<option value="0" '+(0 == $('#choose_cityarea').attr('province_id') ? 'selected="selected"' : '')+'>全部省</option>';
...
function show_city(id,name,type){
	$.post(choose_city,{id:id,name:name,type:type},function(result){
		...
		if(result.error == 0){
			...
			area_dom += '<option value="0" ' + (0 == $('#choose_cityarea').attr('city_id') ? 'selected="selected"' : '') + '>全部市</option>';
			$.each(result.list,function(i,item){
...
function show_area(id,name,type){
	$.post(choose_area,{id:id,name:name,type:type},function(result){
		...
		if(result.error == 0){
			...
			area_dom += '<option value="0" ' + (0 == $('#choose_cityarea').attr('area_id') ? 'selected="selected"' : '') + '>全部区</option>';
			$.each(result.list, function(i, item){
