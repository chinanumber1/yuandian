...
/*************    方法     **************/
	...
	function kalendarCode(codeYear,codeMonth,codeDay,codeFirst){
		kalendar_html = "<table id='kalendar' cellpadding=\"0\" cellspacing=\"0\">\n<tr id='select'>\n<td>\n<div class=\"ul\"><input type='button' id='monthPrev' value='<' /><span id='year'>"+codeYear+"年</span>\n<span id='month'>"+month[codeMonth]+"</span><input type='button' id='monthNext' value='>' /></div></td>\n</tr>\n\n<tr id='week'>\n<td>\n<ul>\n<li class='weekend'>日</li>\n<li>一</li>\n<li>二</li>\n<li>三</li>\n<li>四</li>\n<li>五</li>\n<li class='weekend'>六</li>\n</ul>\n</td>\n</tr>\n\n<tr id='day'>\n<td>\n";
		...
		for(var m=0;m<6;m++){//日期共 4-6 行
			...
			for(var n=0;n<7;n++){//列
				if((7*m+n) < codeFirst || (7*m+n) >= (codeFirst+monthDays[codeMonth])){//某月日历中不存在的日期
					...
					if(7*m+n+1-codeFirst == codeDay){//仅是当天
						kalendar_html += '<li class="today" id="y'+codeYear+'_m'+(codeMonth+1)+'_d'+(7*m+n+1-codeFirst)+'">'+(7*m+n+1-codeFirst)+'</li>';
					}else{//其他日期
						kalendar_html += '<li id="y'+codeYear+'_m'+(codeMonth+1)+'_d'+(7*m+n+1-codeFirst)+'">'+(7*m+n+1-codeFirst)+'</li>';
					}
				}
			}
			kalendar_html += "</ul>\n";
	...
	function y_mChange(obj,stopId,isYear){
		
		if(typeof isYear !== 'undefined'){
			obj.html(stopId+'年');
		}else{
			obj.html(month[stopId]);
		}
		...
		*/
		...
		for(var i=0;i<7*newLine;i++){
			if(i < dateFirstDay || i> (dateTotalDays+dateFirstDay-1)){//日历中 当月不存在的日期
				...
				if(i+1-dateFirstDay == dateCurrentDay){//仅当天
					//dateObj.eq(i).removeClass().addClass('today');
				}else{//其他日期
					dateObj.eq(i).removeClass();
				}
				dateObj.eq(i).text(i+1-dateFirstDay).attr({'id':'y'+someDay['year']+'_m'+(dateMonth+1)+'_d'+(i+1-dateFirstDay)});
			}
		}
		getQD(someDay['year']+'-'+(someDay['month']+1)+'-'+someDay['date']);
	}

/*************    缓存节点和变量     **************/
	var rili_location = $('#wrap_kalendar');//日历代码的位置
	var kalendar_html = '';//记录日历自身代码 的变量
	var yearfloor = 0;//选择年份从1970到当前时间的后10年
...
/*************   将日历代码放入相应位置，初始时显示此处内容      **************/
	...
	/*月-日 设置*/
	var month = new Array('01月','02月','03月','04月','05月','06月','07月','08月','09月','10月','11月','12月');
...
/*************   js写的日历代码中出现的节点     **************/
	...
	var selectYear = $('#year');//选择年份列表
	var listYear = $('#year .selectChange select option');//所有可选年份
	var selectMonth = $('#month');//选择月份列表
...
/***************************/
	...
	monthPrev.bind('click',function(){
		...
		if(monthChange >= 0){//仍在本年内
			...
			y_mChange(selectMonth,monthChange);
			//修改年份
			y_mChange(selectYear,yearChange,true);
			//重新获得 每月天数
