...
<html lang="zh-CN">
	<head>
		...
		<script type="text/javascript">
			var location_url = "<?php echo U('House/ajax_village_list');?>",keyword = "",backUrl="<?php echo U('Home/index');?>",account = "<?php echo ($_GET["account"]); ?>";
...
#cityBtn:after {
	...
	<body>
    <?php if(!$is_app_browser): ?><header class="pageSliderHide">
		<?php if(!defined("IS_INDEP_HOUSE")): ?><div id="backBtn"></div>小区列表
		<?php else: ?>
		<div id="cityBtn" class="link-url" data-url="<?php echo U('Changecity/index');?>"><?php echo ($config["now_select_city"]["area_name"]); ?></div>
			<div style=" float:left; text-align:center; width:80%">小区列表</div><?php endif; ?>
		</header><?php endif; ?>
			...
			<div id="scroller">
				...
				<script id="villageBoxTpl" type="text/html">
					{{# for(var i = 0, len = d.length; i < len; i++){ }}
						<dd class="link-url"	{{# if(d[i].flag){ }} data-url="<?php echo U('House/village_select');?>&village_id={{ d[i].village_id }}&pigcms_id={{ d[i].pigcms_id }}" {{# }else{ }}data-url="<?php echo U('House/village_select');?>&village_id={{ d[i].village_id }}"{{# } }}>
