		...
		<div class="mainbox">
			<div id="nav" class="mainnav_title">
				<ul>
					...
					<a href="javascript:void(0);" onclick="window.top.artiframe('<?php echo U('Slider/cat_add');?>','添加导航分类',500,320,true,false,false,addbtn,'add',true);">添加导航分类</a>
				</ul>
			</div>
					...
					<table width="100%" cellspacing="0">
						...
						<tbody>
							<?php if(is_array($category_list)): if(is_array($category_list)): $i = 0; $__LIST__ = $category_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
										...
										<td><?php if($vo['cat_type']): ?>PC站<?php else: ?>WAP站<?php endif; ?></td>
										...
										<td class="textcenter">
											<?php if(!$vo['is_system']): ?><a href="javascript:void(0);" onclick="window.top.artiframe('<?php echo U('Slider/cat_edit',array('cat_id'=>$vo['cat_id'],'frame_show'=>true));?>','查看导航分类',500,320,true,false,false,false,'add',true);">查看</a> | <a href="javascript:void(0);" onclick="window.top.artiframe('<?php echo U('Slider/cat_edit',array('cat_id'=>$vo['cat_id']));?>','编辑导航分类',500,320,true,false,false,editbtn,'add',true);">编辑</a> | <a href="javascript:void(0);" class="delete_row" parameter="cat_id=<?php echo ($vo["cat_id"]); ?>" url="<?php echo U('Slider/cat_del');?>">删除</a>
											<?php else: ?>
												系统自带，无法操作<?php endif; ?>
										</td>
