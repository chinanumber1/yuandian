	...
	<form id="myform" method="post" action="<?php echo U('Adver/adver_amend');?>" enctype="multipart/form-data">
		...
		<table cellpadding="0" cellspacing="0" class="frame_form" width="100%">
			...
			<tr>
				...
				<td><input type="text" class="input fl" name="sub_name" size="20" value="<?php echo ($now_adver["sub_name"]); ?>" placeholder="请输入副标题" validate="required:true"/></td>
			</tr>
			<?php if($many_city == 1 AND $now_category['cat_key'] != 'levelad'): ?><tr>
					<th width="15%">通用广告</th>
					<td width="35%" class="radio_box">
						<span class="cb-enable"><label class="cb-enable <?php if($now_adver['city_id'] == 0): ?>selected<?php endif; ?>"><span>通用</span><input id="yes" type="radio" name="currency" value="1" <?php if($now_adver['city_id'] == 0): ?>checked="checked"<?php endif; ?> /></label></span>
						<span class="cb-disable"><label class="cb-disable <?php if($now_adver['city_id'] != 0): ?>selected<?php endif; ?>"><span>不通用</span><input id="no" type="radio" name="currency" value="2" <?php if($now_adver['city_id'] != 0): ?>checked="checked"<?php endif; ?> /></label></span>
					</td>
				</tr>
				<tr id="adver_region" <?php if($now_adver['city_id'] == 0): ?>style="display:none;"<?php endif; ?>>
					<th width="15%">所在区域</th>
					<td width="85%" colspan="3" id="choose_cityareass" province_idss="<?php echo ($now_adver['province_id']); ?>" city_idss="<?php echo ($now_adver['city_id']); ?>"></td>
				</tr><?php endif; ?>
			<tr>
				<th width="80">广告现图</th>
            ...
            <tr>
                <th width="80">视频地址</th>
                <td><input type="text" class="input fl" name="video" style="width:200px;" value="<?php echo ($now_adver["video"]); ?>" placeholder="输入视频" tips="不修改请不上传！上传新视频，老视频会被自动删除！"/></td>
            </tr>
			...
			<tr>
				<th width="80">广告排序</th>
				<td><input type="text" class="input fl" name="sort" style="width:80px;" value="<?php echo ($now_adver["sort"]); ?>" validate="maxlength:10,required:true,number:true"/></td>
			</tr>
			<tr>
				<th width="80">广告状态</th>
				<td>
			...
			<tr>
				...
				<td>
					<span class="cb-enable"><label class="cb-enable <?php if($now_adver['complete'] == 1): ?>selected<?php endif; ?>"><span>是</span><input type="radio" name="complete" value="1" <?php if($now_adver['complete'] == 1): ?>checked="checked"<?php endif; ?>/></label></span>
					<span class="cb-disable"><label class="cb-disable <?php if($now_adver['complete'] == 0): ?>selected<?php endif; ?>"><span>否</span><input type="radio" name="complete" value="0" <?php if($now_adver['complete'] == 0): ?>checked="checked"<?php endif; ?>/></label></span>
